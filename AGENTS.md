...
1. `develop` 是唯一开发整合线，`main` 只接收整合线验收通过的版本。
2. 每个新任务先拉取远端，再从最新 `origin/develop` 创建一个短分支 `codex/<任务名>`。普通功能任务不得直接在整合线或 `main` 上开发。
3. 一个任务只对应一个分支、一个工作目录和一个合并请求。开始工作的当天必须推送分支，并建立目标为 `develop` 的草稿合并请求；不能把未推送的本地工作目录当作长期成果保存处。
...
- 只有明确标记为“整合”或“发布”的任务可以合并合并请求、更新 `develop`、把整合线合入 `main`、创建标签、发布版本或上传正式安装包。
